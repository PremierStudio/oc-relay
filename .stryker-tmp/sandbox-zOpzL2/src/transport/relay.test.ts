...
function envelope(overrides: Partial<HandoffEnvelope> = {}): HandoffEnvelope {
  return {
    ...
    session: { id: "ses_src", title: "CASA ops" },
...
describe("receiveHandoff", () => {
  ...
  it("creates the worktree and anchors the envelope", async () => {
    ...
    expect(r.directory).toBe("/r/.worktrees/ops-panel");
    expect(r.anchorPath).toBe(`/r/.worktrees/ops-panel/${HANDOFF_ANCHOR_RELPATH}`);
  ...
  it("round-trips a built envelope through anchor contents", async () => {
