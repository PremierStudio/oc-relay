...
```
     ...
     Tailscale (private mesh)          HTTPS + Cloudflare Access (public door)
                   │                                │
   ┌───────────────┴────────────┐        ┌──────────┴──────────┐
   │ laptop    m3-ultra   nas   │        │  CF Durable Object  │
...
```
you: "move this to m3ultra"            (or tap /relay → pick from list)
relay: commit WIP → push branch
relay: export session event stream (sync/history)
relay: post offer to m3ultra's OC2 API (pair creds, via tailnet)
m3ultra: /sync/replay → session recreated
m3ultra: worktree created (branch opencode/<name>)
m3ultra: relay apply ← env.json (1Password SA resolves secrets silently)
phone: 🔔 "CASA ops is live on m3ultra" → tap → working
```
...
| 2 | Transport lib: sync-protocol client + export/import fallback + worktree create + **in-band handoff manifest** (context payload format) | same gates |
| 3 | `/handoff` plugin command + `relay send`/`receive` CLI — E2E machine↔machine | demo |
| 4 | Discovery via Tailscale services + fleet config from pair creds | demo |
