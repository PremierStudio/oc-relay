...
describe("planWorktree", () => {
  it("plans branch and directory from the slug with defaults", () => {
    const p = planWorktree({ repoDir: "/repos/SampleApp", name: "Ops Panel!" });
    expect(p).toEqual({
      branch: "opencode/ops-panel",
      directory: "/repos/SampleApp/.worktrees/ops-panel",
      args: ["worktree", "add", "-b", "opencode/ops-panel", "/repos/SampleApp/.worktrees/ops-panel"],
    });
  });
