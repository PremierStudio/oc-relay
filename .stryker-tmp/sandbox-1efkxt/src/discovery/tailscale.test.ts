...
describe("parseTailscaleStatus", () => {
  ...
  it("sorts peers regardless of insertion order", () => {
    const peers = parseTailscaleStatus({
      Peer: {
        zLast: { HostName: "zulu", DNSName: "zulu.invalid." },
        mMid: { HostName: "mike", DNSName: "mike.invalid." },
        aFirst: { HostName: "alpha", DNSName: "alpha.invalid." },
      },
    });
    expect(peers.map((p) => p.host)).toEqual(["alpha", "mike", "zulu"]);
  });

  it("falls back to dns when HostName is a non-string or empty", () => {
    const peers = parseTailscaleStatus({
      Peer: {
        numbered: { HostName: 42, DNSName: "numbered.invalid." },
        emptied: { HostName: "", DNSName: "emptied.invalid." },
      },
    });
    expect(peers.map((p) => p.host)).toEqual(["emptied.invalid", "numbered.invalid"]);
  });
  ...
  it("omits ip for ipv6-only peers", () => {
    ...
    expect(candidateBaseUrls(peers[0]!)).toStrictEqual([
...
describe("candidateBaseUrls", () => {
  ...
  it("defaults to http on the OC2 port, dns first then ip", () => {
    expect(candidateBaseUrls(peer)).toStrictEqual([
  ...
  it("puts https magic-dns first when requested", () => {
    expect(candidateBaseUrls(peer, { https: true })).toStrictEqual([
      "https://build-server.tailnet-example.ts.net",
      "https://build-server.tailnet-example.ts.net:49374",
      "http://100.64.0.11:49374",
    ]);
  });

  it("honors a custom port", () => {
    expect(candidateBaseUrls(peer, { port: 8080 })).toStrictEqual([
