...
describe("Oc2SyncClient", () => {
  const clientOpts = { baseUrl: "http://build-server:49374/", credentials: creds };
  ...
  it("sends authenticated POSTs with JSON bodies to default endpoints", async () => {
    ...
    expect(calls[0]?.url).toBe(`http://build-server:49374${DEFAULT_SYNC_ENDPOINTS.history}`);
  ...
  it("replay sends the event batch alongside the source session id", async () => {
    ...
    expect(calls[0]?.url).toBe(`http://build-server:49374${DEFAULT_SYNC_ENDPOINTS.replay}`);
  ...
  it("replay rejects a null body as SyncError without crashing", async () => {
    const { fetch } = fakeFetch(() => okResponse(null));
    const err = await new Oc2SyncClient({ ...clientOpts, fetch })
      .replay("ses_1", [])
      .catch((e: unknown) => e);
    expect(err).toBeInstanceOf(SyncError);
    expect((err as SyncError).body).toBe("null");
  });

  it("replay surfaces non-object bodies through SyncError faithfully", async () => {
    const textBody: FetchLike = async () => ({ ok: true, status: 200, text: async () => "garbage{" });
    const err = await new Oc2SyncClient({ ...clientOpts, fetch: textBody })
      .replay("ses_1", [])
      .catch((e: unknown) => e);
    expect(err).toBeInstanceOf(SyncError);
    // the raw non-JSON text is carried through for diagnostics
    expect((err as SyncError).body).toContain("garbage{");
  });
  ...
  it("honors endpoint overrides for beta drift", async () => {
    ...
    expect(calls[0]?.url).toBe("http://build-server:49374/experimental/sync/replay");
  });
