...
export interface EndpointAuth {
  ...
  password?: string;
  /** Name of the env variable holding the password — preferred. */
  passwordEnv?: string;
}
...
function parseEndpointAuth(
  ...
  const out: { baseUrl?: string; username?: string; password?: string } = {};
  ...
  if (passwordEnv !== undefined) {
    if (!isNonEmptyString(passwordEnv)) {
      ...
      if (resolved === undefined) {
        errors.push({ path: `${prefix}.passwordEnv`, message: `env variable not set: ${passwordEnv}` });
      } else {
        out.password = resolved;
      }
    }
  }
  return out;
