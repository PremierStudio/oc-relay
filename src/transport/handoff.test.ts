...
describe("parseHandoffEnvelope", () => {
  ...
  it.each([
    ...
    [{ ...validEnvelope, session: { title: 42 } }, "session.title"],
    [{ ...validEnvelope, context: "nope" }, "context"],
    [{ ...validEnvelope, context: { summary: 42 } }, "context.summary"],
    [{ ...validEnvelope, refs: [{ label: "x", detail: 7 }] }, "refs.0.detail"],
    [null, ""],
    ["string", ""],
...
describe("buildHandoffEnvelope", () => {
  ...
  it("names the offending field paths in the construction error", () => {
    expect(() =>
      buildHandoffEnvelope({
        sourceHost: "",
        repo: "repo",
  ...
  it("leaves the session key entirely absent when none is given", () => {
    const env = buildHandoffEnvelope({
      sourceHost: "laptop",
      repo: "repo",
      branch: "b",
    ...
    expect(Object.hasOwn(env, "session")).toBe(false);
    expect(JSON.parse(JSON.stringify(env)).session).toBeUndefined();
  });
});
